...
int main()
{
    ...
    freopen("testin_cd.in", "r", stdin);
    freopen("testout_cd.out", "w", stdout);
    for ()
        ...
        if (lon1 != lon3 || lat1 != lat3)
        {
            ...
            for (int i=0; i<MAX; i++)
            {
                comparable_geographic_distance_result = bg::formula::compare_length_haversine<double>
        ...
        else if (lon1 == lon3 && lat1 == lat3)
        {
            ...
            for (int i=0; i<MAX; i++)
            {
                comparable_geographic_distance_result = bg::formula::flat_approximation<double>::apply(P1, P2, P4, earth); 
            }
            std::clock_t comparable_geographic_distance_stop = std::clock();
            secs_comparable_geographic_distance = double(comparable_geographic_distance_stop -
                                                         comparable_geographic_distance_start)
        ...
        std::cout << "P1: " << lon1 << ", " << lat1 << " P2: " << lon2 << ", " << lat2 << std::endl;
        std::cout << "P3: " << lon3 << ", " << lat3 << " P4: " << lon4 << ", " << lat4 << std::endl;
